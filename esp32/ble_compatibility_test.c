...
#define SAMPLE_DEVICE_NAME          "BLE_TEST"
...
static void state1_callback(void* arg);
static void state2_callback(void* arg);
...
void app_main(void)
{
    ...
    // turn off backlight
    ESP_LOGI(EXAMPLE_TAG, "backlight off");
    //_wait_for_user();
    vTaskDelay(1000 / portTICK_PERIOD_MS);
    i2c_lcd1602_set_backlight(lcd_info, false);

    // turn on backlight
    ESP_LOGI(EXAMPLE_TAG, "backlight on");
    //_wait_for_user();
    vTaskDelay(1000 / portTICK_PERIOD_MS);
    i2c_lcd1602_set_backlight(lcd_info, true);

    ESP_LOGI(EXAMPLE_TAG, "enable display");
    i2c_lcd1602_set_display(lcd_info, true);

    uint8_t notify[2];
    const esp_timer_create_args_t state1_timer_args = {
            .callback = &state1_callback,
            /* argument specified here will be passed to timer callback function */
            .name = "state1"
    };
    esp_timer_handle_t state1_timer;
    ESP_ERROR_CHECK(esp_timer_create(&state1_timer_args, &state1_timer));

    const esp_timer_create_args_t state2_timer_args = {
            .callback = &state2_callback,
            /* argument specified here will be passed to timer callback function */
            .name = "state2"
    };
    esp_timer_handle_t state2_timer;
    ESP_ERROR_CHECK(esp_timer_create(&state2_timer_args, &state2_timer));
    ...
    ESP_ERROR_CHECK(esp_timer_start_periodic(pulse_timer, 10000));
    ESP_ERROR_CHECK(esp_timer_start_periodic(state1_timer, 1000000));
    ESP_ERROR_CHECK(esp_timer_start_periodic(state2_timer, 1000000));
    ...
    while (1)
    {
        ...
        case 0:
            if(isConnected){
                esp_err_t res=esp_ble_gatts_close(mGatts_if,0);
            }
            // printf("gi cung duoc");
            count1=0;
            count2=0;
            if(humanDetect()) myCase=1;
            break;
        case 1:
            sprintf(buf, "HR: %.02f BPM", result.heart_bpm);
        ...
        case 2:
            ...
            break;
        case 3:
            notify[0]= result.heart_bpm;
            notify[1]=(result.heart_bpm-(int) result.heart_bpm)*100;
            printf("%d,%d",notify[0],notify[1]);
            esp_ble_gatts_set_attr_value(gatt_db_handle_table[IDX_CHAR_VAL_C],sizeof(notify), notify);
            esp_ble_gatts_send_indicate(mGatts_if, 0, gatt_db_handle_table[IDX_CHAR_VAL_C], 2, notify, false);
            vTaskDelay(1000 / portTICK_PERIOD_MS);
        default:
            break;
        }
